...
pred sendPacketAckCheck[s1, s2: SystemState]{
...
run receiveCorruptedPacket for 3
...
pred receiveCorruptedPacket[s1, s2: SystemState]{
		...
		(one p:Packet | (p in s2.inFlight) and (p.insideData in Ack) and p.sequence = s1.lastRecievedSequence)
